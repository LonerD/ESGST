    ...
    if (Context) {
        ...
        for (I = 0, NumGiveaways = Giveaways.length; I < NumGiveaways; ++I) {
            ...
            if (Timestamp < (new Date().getTime())) {
                ...
                if (!User.UGD[UGD.Key + "Timestamp"]) {
                    User.UGD[UGD.Key + "Timestamp"] = Timestamp + 1;
                }
...
function refreshPRPoints(Points, PR, Title, Background) {
    ...
    if (Background) {
        queueRequest({}, "xsrf_token=" + ESGST.XSRFToken + "&do=entry_insert", "/ajax.php", Callback);
    } else {
...
function displayGDCBPPopup(GP, Enter) {
    ...
    makeRequest(null, GP.URL, null, function(Response) {
        ...
        if (Description || (!Description && ((Enter && !GM_getValue("GDCBP_D")) || !Enter))) {
            Popup = createPopup(true);
...
function setATTimestamp(Context) {
    if (!Context.textContent.match(/\*/) && !Context.closest(".giveaway__column--width-fill.text-right")) {
